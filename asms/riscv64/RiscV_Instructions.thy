...
abbreviation \<open>sd w rs1 imm rs2 \<equiv> \<lparr> addr = w, size = (2 \<Colon> 64), code = [mem := (mem with [(rs1 :\<^sub>t imm\<langle>64\<rangle>) + (imm \<Colon> 64), el]:u64 \<leftarrow> (rs2 :\<^sub>t imm\<langle>64\<rangle>))] \<rparr>\<close>

lemma step_sd:
  assumes decode_sd: \<open>\<And>\<Delta> var. (\<Delta>, (w), var) \<mapsto>\<^sub>b\<^sub>i\<^sub>l sd w rs1 imm rs2\<close>
    ...
    shows \<open>(\<Delta>, (w), var) \<leadsto>\<^sub>b\<^sub>i\<^sub>l (\<Delta>(mem \<mapsto> storage_el64 mem' ((mem_addr \<Colon> 64) +\<^sub>b\<^sub>v (imm \<Colon> 64)) (val \<Colon> 64)), (w) +\<^sub>b\<^sub>v (2 \<Colon> 64), var)\<close>
  apply (insert assms(2-))
...
abbreviation \<open>jalr w rs1 ret target \<equiv> \<lparr> addr = w, size = (4 \<Colon> 64), code = [X6 := (ret \<Colon> 64), jmp ((rs1 :\<^sub>t imm\<langle>64\<rangle>) + (target \<Colon> 64))]\<rparr>\<close>

lemma step_jalr:
  assumes decode_jalr:  \<open>\<And>\<Delta> var. (\<Delta>, w, var) \<mapsto>\<^sub>b\<^sub>i\<^sub>l jalr w rs1 ret target\<close>
...
lemma step_mv: 
  assumes decode_mv:  \<open>\<And>\<Delta> var. (\<Delta>, w, var) \<mapsto>\<^sub>b\<^sub>i\<^sub>l mv w rd rs1\<close>
      and \<open>((rs1 :\<^sub>t imm\<langle>64\<rangle>), val \<Colon> 64) \<in>\<^sub>\<Delta> \<Delta>\<close> 
...
abbreviation \<open>lw w rd rs1 imm \<equiv> \<lparr> addr = w, size = (4 \<Colon> 64), code = [(rd :\<^sub>t imm\<langle>64\<rangle>) := (extend:64[mem[(rs1 :\<^sub>t imm\<langle>64\<rangle>) + (imm \<Colon> 64), el]:u32])]\<rparr>\<close>
...
lemma step_lw: 
  assumes decode_lw:  \<open>\<And>\<Delta> var. (\<Delta>, w, var) \<mapsto>\<^sub>b\<^sub>i\<^sub>l lw w rd rs1 imm\<close>
    ...
    shows \<open>(\<Delta>, (w), var) \<leadsto>\<^sub>b\<^sub>i\<^sub>l (\<Delta>((rd :\<^sub>t imm\<langle>64\<rangle>) \<mapsto> (val \<Colon> 64)), (w) +\<^sub>b\<^sub>v (4 \<Colon> 64), var)\<close>
...
abbreviation \<open>sw w rs1 imm rs2 \<equiv> \<lparr> addr = w, size = (4 \<Colon> 64), code = [mem := (mem with [(rs1 :\<^sub>t imm\<langle>64\<rangle>) + (imm \<Colon> 64), el]:u32 \<leftarrow> (low:32[(rs2 :\<^sub>t imm\<langle>64\<rangle>)]))]\<rparr>\<close>

lemma step_sw: 
  assumes decode_sw:  \<open>\<And>\<Delta> var. (\<Delta>, w, var) \<mapsto>\<^sub>b\<^sub>i\<^sub>l sw w rs1 imm rs2\<close>
      and \<open>((rs1 :\<^sub>t imm\<langle>64\<rangle>), mem_addr \<Colon> 64) \<in>\<^sub>\<Delta> \<Delta>\<close> and \<open>((rs2 :\<^sub>t imm\<langle>64\<rangle>), val \<Colon> 64) \<in>\<^sub>\<Delta> \<Delta>\<close> and \<open>(mem, mem') \<in>\<^sub>\<Delta> \<Delta>\<close> and \<open>type mem' = mem\<langle>64, 8\<rangle>\<close>
    shows \<open>(\<Delta>, (w), var) \<leadsto>\<^sub>b\<^sub>i\<^sub>l (\<Delta>(mem \<mapsto> storage32 mem' (((mem_addr + imm) mod 18446744073709551616) \<Colon> 64) ((take_bit 32 val) \<Colon> 64)), (w) +\<^sub>b\<^sub>v (4 \<Colon> 64), var)\<close>
  apply (insert assms(2-))
...
lemma step_beqz:
  assumes decode_beqz: \<open>\<And>\<Delta> var. (\<Delta>, w, var) \<mapsto>\<^sub>b\<^sub>i\<^sub>l \<lparr> addr = (w), size = (2 \<Colon> 64), code = [temp :\<^sub>t imm\<langle>1\<rangle> := BinOp (rs1 :\<^sub>t imm\<langle>64\<rangle>) (LOp Eq) (0 \<Colon> 64), IfThen (temp :\<^sub>t imm\<langle>1\<rangle>) [jmp (offset \<Colon> 64)]]\<rparr>\<close>
...
lemma step_ld:
  assumes decode_ld: \<open>\<And>\<Delta> var. (\<Delta>, w, var) \<mapsto>\<^sub>b\<^sub>i\<^sub>l \<lparr> addr = (w), size = (4 \<Colon> 64), code = [rd := (mem[(rs1 :\<^sub>t imm\<langle>64\<rangle>) + (imm \<Colon> 64), el]:u64)]\<rparr>\<close>
      and \<open>((rs1 :\<^sub>t imm\<langle>64\<rangle>), (mem_addr \<Colon> 64)) \<in>\<^sub>\<Delta> \<Delta>\<close> and \<open>(mem, mem') \<in>\<^sub>\<Delta> \<Delta>\<close>
      and load: \<open>\<Delta> \<turnstile> (Val mem')[(mem_addr \<Colon> 64) +\<^sub>b\<^sub>v (imm \<Colon> 64), el]:u64 \<leadsto>* (ptr \<Colon> 64)\<close>
    shows \<open>(\<Delta>, (w), var) \<leadsto>\<^sub>b\<^sub>i\<^sub>l (\<Delta>(rd \<mapsto> ptr \<Colon> 64), (w) +\<^sub>b\<^sub>v (4 \<Colon> 64), var)\<close>
  apply (insert assms(2-3))
...
lemma step_cld:
  assumes decode_cld: \<open>\<And>\<Delta> var. (\<Delta>, w, var) \<mapsto>\<^sub>b\<^sub>i\<^sub>l \<lparr> addr = (w), size = (2 \<Colon> 64), code = [rd := (mem[(rs1 :\<^sub>t imm\<langle>64\<rangle>) + (imm \<Colon> 64), el]:u64)]\<rparr>\<close>
      and \<open>((rs1 :\<^sub>t imm\<langle>64\<rangle>), (mem_addr \<Colon> 64)) \<in>\<^sub>\<Delta> \<Delta>\<close> and \<open>(mem, mem') \<in>\<^sub>\<Delta> \<Delta>\<close>
      and load: \<open>\<Delta> \<turnstile> (Val mem')[(mem_addr \<Colon> 64) +\<^sub>b\<^sub>v (imm \<Colon> 64), el]:u64 \<leadsto>* (ptr \<Colon> 64)\<close>
    shows \<open>(\<Delta>, (w), var) \<leadsto>\<^sub>b\<^sub>i\<^sub>l (\<Delta>(rd \<mapsto> ptr \<Colon> 64), (w) +\<^sub>b\<^sub>v (2 \<Colon> 64), var)\<close>
  apply (insert assms(2-3))
...
lemma step_j:
  assumes decode_j: \<open>\<And>\<Delta> var. (\<Delta>, w, var) \<mapsto>\<^sub>b\<^sub>i\<^sub>l \<lparr> addr = (w), size = (2 \<Colon> 64), code = [jmp (offset \<Colon> 64)]\<rparr>\<close>
    shows \<open>(\<Delta>, (w), var) \<leadsto>\<^sub>b\<^sub>i\<^sub>l (\<Delta>, (offset \<Colon> 64), var)\<close>
  by (solve_progI decoder: decode_j)

subsection \<open>NOP\<close>

lemma step_nop:
  assumes decode_nop: \<open>\<And>\<Delta> var. (\<Delta>, w, var) \<mapsto>\<^sub>b\<^sub>i\<^sub>l \<lparr> addr = (w), size = (2 \<Colon> 64), code = []\<rparr>\<close>
    shows \<open>(\<Delta>, (w), var) \<leadsto>\<^sub>b\<^sub>i\<^sub>l (\<Delta>, (w) +\<^sub>b\<^sub>v (2 \<Colon> 64), var)\<close>
...
(* address + offset *)
lemma step_bge_true:
  assumes decode_bge: \<open>\<And>\<Delta> var. (\<Delta>, w, var) \<mapsto>\<^sub>b\<^sub>i\<^sub>l \<lparr> addr = (w), size = (4 \<Colon> 64), code = [(ivar :\<^sub>t imm\<langle>1\<rangle>) := (rs1 :\<^sub>t imm\<langle>64\<rangle>) le (rs2 :\<^sub>t imm\<langle>64\<rangle>), IfThen (ivar :\<^sub>t imm\<langle>1\<rangle>) [jmp (offset \<Colon> 64)]]\<rparr>\<close>
  ...
  by (insert assms(2-3), solve_prog_intI assumption simp: leq decoder: decode_bge)
